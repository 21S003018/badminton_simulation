...
class Environment(gym.Env):
    def __int__(self):
        ...
        self.init_state = self.action_space.sample()
    ...
    def step(self, action):
        # update state
        if action == 0:  # up
            self.current_state[0] = max(self.current_state[0] - 1, 0)
        elif action == 1:  # forward
            self.current_state[1] = min(
                self.current_state[1] + 1, self.cols - 1)
        elif action == 2:  # down
            self.current_state[0] = min(
                self.current_state[0] + 1, self.rows - 1)
        else:
...
class Agent(Entity):
    def __init__(self, index, time_scale):
        ...
        try:
            self.palyer_speed_model = pd.read_csv(
                'Data/player_speed_model.csv', index_col=0)
        except:
            self.palyer_speed_model = pd.read_csv(
                r'rl-environment\0816086\Data\player_speed_model.csv', index_col=0)
    ...
    def step(self):
        ...
        distance_to_target_pos = np.linalg.norm(
...
class World(object):
    ...
    def step(self, action_n):
        ...
        if self.init_step:
            if self.discrete_court:
                ...
                landing_pos = self.area_mapping(
                    1 if self.player == 0 else 0, ball_target_area)
            else:
            ...
            if ball.serve:
                ...
                ball.launching(ball_type, landing_pos,
                               landing_height, launching_height)
            else:
                # 不回擊
            ...
            for ind, act in enumerate(action_n):
                ...
                if self.discrete_court:
                    ...
                    agent_target_pos = self.area_mapping(
                        ind, agent_target_area)
                else:

                    agent_target_pos = act['player_location']
                # print(agent_target_pos)
                agent.initialize_step(
                    role, self.decide_defend_location, agent_target_pos, ball)
    ...
    def check_step_done(self, ball, catcher):
        distance_between_ball_and_catcher = np.linalg.norm(
            catcher.state.p_pos-ball.state.p_pos)
        # Check return
...
class Ball(Entity):
    def __init__(self, n, g, time_scale):
        ...
        self.ball_type_table = pd.read_excel('Data/ball_type_table.xlsx')
        self.ball_initial_state_model = pd.read_csv(
            'Data/initial_state/ball_initial_state_model.csv', index_col=0)
        self.ball_type_frame_distribution = pd.read_csv(
            'Data/ball_type_model.csv', index_col=0)
        self.hit_height = pd.read_csv(
            'Data/hit_height.csv', index_col='type')
        self.hit_range = pd.read_csv(
            'Data/hit_range.csv', index_col='type')
    ...
    def launching(self, ball_type, landing_pos, landing_height, init_height=None):
        ...
        assert not (landing_pos == None).any(
        ), 'Fail to return bacause landing position {} is invalid'.format(landing_pos)
        assert landing_height is not None, 'Fail to return bacause landing height {} is invalid'.format(
            landing_height)
        assert not np.isnan(
            landing_height), 'Fail to return bacause landing height {} is invalid'.format(landing_height)
       ...
       # print(landing_height)
        ...
        speed, angle_arc = self.__generate_deterministic_speed_and_angle(
            ball_type, distance, landing_height-self.state.p_height)
